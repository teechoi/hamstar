...
export default function SettingsPage() {
  ...
  const [buttonLabels, setButtonLabels] = useState<Record<string, string>>(DEFAULT_BUTTON_LABELS)

  useEffect(() => {
    fetch('/api/admin/settings').then((r) => r.json()).then((data) => {
      setSettings(data)
      setButtonLabels({ ...DEFAULT_BUTTON_LABELS, ...(data.buttonLabels ?? {}) })
    })
  }, [])
  ...
  if (!settings) return <div style={{ padding: 32, color: T.textMuted }}>Loading...</div>
