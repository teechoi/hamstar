  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 28px' }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <h2 style={{ fontFamily: 'Georgia, serif', fontSize: 40, fontWeight: 900, color: T.text, marginTop: 10, marginBottom: 8, letterSpacing: -1 }}>
          Meet the Racers
        </h2>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 32, flexWrap: 'wrap' }}>
        {PETS.map((pet) => (
          <button key={pet.id} onClick={() => setSelected(pet)} style={{
            ...
            <div style={{ width: 44, height: 44, borderRadius: 10, background: selected.id === pet.id ? pet.color : T.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, overflow: 'hidden', transition: 'all 0.15s' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        {/* LEFT: Pet card */}
        <div style={{ background: T.card, border: `2px solid ${selected.color}`, borderRadius: 20, overflow: 'hidden', boxShadow: `6px 6px 0px ${selected.color}` }}>
          <div style={{ height: 6, background: selected.color }} />
          <div style={{ padding: 28 }}>
            {/* Identity */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
              <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
                <div style={{ width: 80, height: 80, borderRadius: 20, background: selected.color + '18', border: `3px solid ${selected.color}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 46, animation: 'petIdle 3s ease-in-out infinite', overflow: 'hidden' }}>
                  {selected.image ? <img src={selected.image} alt={selected.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : selected.emoji}
                </div>
                <div>
                  <div style={{ fontSize: 26, fontWeight: 900, color: T.text, letterSpacing: -0.5 }}>{selected.name}</div>
              ...
              <div style={{ width: 52, height: 52, borderRadius: 12, background: selected.color, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <span style={{ fontSize: 16, fontWeight: 900, color: '#fff' }}>#{selected.number}</span>
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 20 }}>
              {[['Speed', '🏃', selected.speed], ['Chaos', '⚡', selected.chaos], ['Wins', '🏆', selected.wins]].map(([l, icon, v]) => (
                <div key={String(l)} style={{ background: T.bg, border: `2px solid ${T.border}`, borderRadius: 12, padding: '12px 6px', textAlign: 'center' }}>
                  <div style={{ fontSize: 16, marginBottom: 4 }}>{icon}</div>
                  <div style={{ fontSize: 26, fontWeight: 900, color: T.text }}>{v}</div>
            ...
            {/* Lifestyle bars */}
            <div style={{ background: T.bg, border: `2px solid ${T.border}`, borderRadius: 14, padding: '14px 16px', marginBottom: 20 }}>
    ...
    <div style={{ background: T.card, border: `2px solid ${T.border}`, borderRadius: 18, padding: 24 }}>
      ...
      {PETS.map((p, i) => (
        <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '10px 0', borderBottom: i < PETS.length - 1 ? `1px solid ${T.border}` : 'none' }}>
          ...
          <button onClick={() => onSelect(p)} style={{ padding: '5px 10px', background: selected.id === p.id ? p.color + '18' : T.bg, border: `1.5px solid ${p.color}`, borderRadius: 6, color: p.color, fontSize: 10, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit' }}>
